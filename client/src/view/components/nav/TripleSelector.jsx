...
export const TripleSelector = ({ handleChange, value, user1Name, user2Name }) => {
    ...
    <Wrapper>
      {options.map((d, i) => (
        <Option key={i} onClick={() => handleChange(d)} selected={selected ? selected === d : i === 0}>
          ...
          {d === "user1" ? user1Name : d === "user2" ? user2Name : "combined"}{" "}
          {/*we need to display the name, but use user1 or user2 behind the scenes */}
        </Option>
      ))}
